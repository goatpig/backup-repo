...
class MemPool
{
	...
	***/
	...
	public:
		BufferHeader **BH;
		static const int memsize = 20000;
		unsigned int freemem, total, reserved, slack, fmabs, nBH;
		std::atomic_bool ab;
		byte defrag;
		unsigned int i; 
		BufferHeader *bhtmp; //global bhtmp could result in unexpected behavior, should use local bhtmp instead. However, private pools are locked and thus function sequentially. Random buffer query locks the pool as well.

		/***reserved: marks how deep the pool buffer has been assigned for. GetBuffer will always assign new BH starting pool + reserved, and increment reserved by size
		...
		BufferHeader *PopBH(byte *start, unsigned int size, unsigned int *sema)
		{
			...
			bhtmp->total = size;
			bhtmp->move = 0;
			/*** Do no update freemem in popbh. We assume this function is used to assign memeory that has already been reserved through getprivatepool(). Use GetBuffer otherwise 
			The general rule would imply PopBH doesn't manage any pool level information, only BH data***/
			//freemem-=size;
